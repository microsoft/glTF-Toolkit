...
std::wstring FileSystem::GetFilenameWithoutExtension(const std::wstring& path)
{
    char sep = '\\';
    ...
    if (FAILED(PathCchRemoveExtension(&filename[0], filename.length() + 1)))
    {
        throw std::invalid_argument("Invalid input file extension.");
    }

    return std::move(std::wstring(&filename[0]));
}
